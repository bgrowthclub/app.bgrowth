...
// (`relatedProductIds`) via ProductCatalogService.getRelatedProducts(),
// never a hardcoded list. Reuses BusinessSystemCard for any related product that resolves to
...
export default function RelatedProductsPanel({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    productCatalogService.getRelatedProducts(productId).then((products) => {
